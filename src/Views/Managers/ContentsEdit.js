...
export default class ContentsEdit extends React.Component {
    ...
    // _GoodsInsert = params => { // 1-insert, 2-lang change, 3-name change, 4-currency, 5-price , 6-delete
    //     if (params.type == 1) {
    //         const obj = {
    //             'no': this.state.goodsDatas.length, 'name': '', 'currency': 'KRW', 'price': '', 'language': ''
    //         }
    ...
    //                 item => params.no === item.no
    //                     ? { no: item.no, language: params.language, name: item.name, currency: item.currency, price: item.price }
    //                     : item
    //             )
    //         })
    //     } else if (params.type == 3) {
    ...
    //             goodsDatas: this.state.goodsDatas.map(
    //                 item => params.no === item.no
    //                     ? { no: item.no, language: item.language, name: params.name, currency: item.currency, price: item.price }
    //                     : item
    //             )
    //         })
    ...
    //             goodsDatas: this.state.goodsDatas.map(
    //                 item => params.no === item.no
    //                     ? { no: item.no, language: item.language, name: item.name, currency: params.currency, price: item.price }
    //                     : item
    //             )
    //         })
    ...
    //             goodsDatas: this.state.goodsDatas.map(
    //                 item => params.no === item.no
    //                     ? { no: item.no, language: item.language, name: item.name, currency: item.currency, price: params.price }
    //                     : item
    //             )
    //         })
    //     } else {
    //         const newList = this.state.goodsDatas.filter((item) => item.no !== params.no);
    //         this.setState({ goodsDatas: newList })
    //     }
    // }
    ...
    async _ContentsUpdate() {
        ...
        formBody = JSON.stringify({
            ...
            "user_no": User.userNo,
            "hashtags": "\'" + JSON.stringify(this.state.tagDatas) + "\'",
            "categories": "\'" + JSON.stringify(this.state.selectCategoryList) + "\'",
            "place_name": "\'" + JSON.stringify(placeObj) + "\'",
            "title": "\'" + JSON.stringify(placeSingleInfoObj) + "\'",
            "content": "\'" + JSON.stringify(placeInfoObj) + "\'",
        ...
        console.log(json)
        if (json.length > 0) {
            this.state.placeNo = this.props.route.params.placeNo
    ...
    _UpdatePlaceImageUrls() {
        ...
        console.log(this.state.photoDatas)
        formdata.append('place_no', this.state.placeNo)
    ...
    async _ContentsPlace() {
        ...
        const json = await NetworkCall.Select(url, formBody)
        console.log(json)
        const imgUrls = JSON.parse(json[0].image_urls)
        const coverUrl = JSON.parse(json[0].image_representative)
        const creatorUrl = JSON.parse(json[0].image_creator)
        const categoryToInt = [];
        JSON.parse(json[0].categories.replace((/'/gi), '')).map((item, index) => (categoryToInt.push(item)))
        const placeName = JSON.parse(json[0].place_name);
        const placeTitle = JSON.parse(json[0].title);
        const placeContents = JSON.parse(json[0].content);
        const productList = JSON.parse(json[0].product_list)
        const dateTimeList = JSON.parse(json[0].operate_hours)
        const hashTagList = json[0].hashtags
        //장소이름
        if (placeName.default != undefined && placeName.default.length > 0) {
            const placeObj = {
        ...
        if (placeName.ko != undefined && placeName.ko.length > 0) {
            const placeObj = {
                no: this.state.placeDatas.length,
                language: 'ko',
                place: placeName.ko
        ...
        if (placeName.en != undefined && placeName.en.length > 0) {
            const placeObj = {
                no: this.state.placeDatas.length,
                language: 'en',
                place: placeName.en
        ...
        if (placeName.ja != undefined && placeName.ja.length > 0) {
            const placeObj = {
                no: this.state.placeDatas.length,
                language: 'ja',
                place: placeName.ja
        ...
        if (productList != null) {
            for (let i = 0; i < productList.length; i++) {
                ...
                const obj = { no: i, currency: productList[i].currency, price: productList[i].price, default: productList[i].name, trans: transList }
                this.state.goodsDatas.push(obj)
            }
            // if (productList.default != undefined && productList.default.length > 0) {
            //     for (let i = 0; i < productList.default.length; i++) {
            //         const productObj = {
            //             no: this.state.goodsDatas.length,
            //             language: 'default',
            //             name: productList.default[i].name,
            //             currency: productList.default[i].currency,
            //             price: productList.default[i].price,
            //         }
            //         this.state.goodsDatas.push(productObj)
            //     }
            // }
            ...
            //             currency: productList.ko[i].currency,
            ...
            //             currency: productList.en[i].currency,
            ...
            //             currency: productList.ja[i].currency,
        ...
        console.log('length : ' + hashTagList.length)
        for (let i = 0; i < hashTagList.length; i++) {
            if (hashTagList[i].name != undefined && hashTagList[i].name.length > 0) {
                this.state.tagDatas.push(hashTagList[i].name)
            }
        ...
        this.setState({
            mapSearchText: json[0].address,
            addressDetail: json[0].address_detail,
            selectCategoryList: categoryToInt,
            lat: json[0].lat,
            lng: json[0].lng,
            isFetching: false,
            selectedCountryNo: json[0].country,
            selectedCityNo: json[0].city,
            selectedRegionNo: json[0].town,
            selectedCountry: json[0].country == -1 ? '' : Utils.Grinder(User.country.filter((el) => el.country_no == json[0].country)[0]),
            selectedCity: json[0].city == -1 ? '' : Utils.Grinder(User.city.filter((el) => el.city_no == json[0].city)[0]),
            selectedRegion: json[0].town == -1 ? '' : Utils.Grinder(User.region.filter((el) => el.town_no == json[0].town)[0]),
            placeTel: json[0].tel,
            placeWeb: json[0].web,
            creatorName: json[0].creator_name,
            creatorUrl: json[0].creator_url,
            photoDatas: imgUrls.map((item, index) => ({ path: ServerUrl.Server + imgUrls[index], fileName: imgUrls[index], mime: 'image/' + imgUrls[index].split('.').pop() })),
            ...
            creatorDatas: creatorUrl.map((item, index) => ({ path: ServerUrl.Server + creatorUrl[index], fileName: creatorUrl[index], mime: 'image/' + creatorUrl[index].split('.').pop() })),
                ...
                <View style={{ width: '100%', height: '100%', backgroundColor: Colors.colorFFFFFF }}>
                    ...
                    <TouchableOpacity onPress={() => this.props.navigation.goBack()}>
                        <View style={{ paddingLeft: 16, alignItems: 'center', paddingTop: 11, flexDirection: 'row' }}>
                            ...
                            <Text style={{ fontSize: 20, color: Colors.color000000, fontFamily: 'Raleway-Bold', includeFontPadding: false, }}>{I18n.t('managerContentsEdittTitle')}</Text>
                        </View>
                    </TouchableOpacity>
                    ...
                    <KeyboardAwareScrollView style={{ marginTop: 16 }} enableOnAndroid={true} extraHeight={Platform.OS == 'ios' ? 100 : 65} showsHorizontalScrollIndicator={false} showsVerticalScrollIndicator={false}>
                                ...
                                {this.state.placeDatas.map((item, index1) => (
                                    <View style={{ flexDirection: 'row', height: 48, width: '100%', marginTop: 12, }}>
                                        {index1 != 0 && (
                                            <View style={{ flex: 0.3, borderRadius: 4, borderWidth: 1, borderColor: Colors.colorD0D0D0, justifyContent: 'center' }}>
                                                <SelectDropdown
                                                    ...
                                                    defaultButtonText={(item.language == undefined || item.language.length == 0) ? I18n.t('managerContentsInsertLanguage') : this.state.lang == 'ko' ? (item.language == 'ko' ? '한국어' : item.language == 'en' ? '영어' : '일본어') : (item.language == 'ko' ? 'Korean' : item.language == 'en' ? 'English' : 'Japanese')}
                                        ...
                                        <View style={{ flex: index1 == 0 ? 1 : 0.7, borderRadius: 4, borderWidth: 1, borderColor: Colors.colorD0D0D0, justifyContent: 'center', marginLeft: index1 == 0 ? 0 : 15, paddingLeft: 18 }}>
                                            <TextInput onChangeText={(text) => this._PlaceInsert({ type: 3, place: text, no: index1 })} value={item.place} style={{ padding: 0, margin: 0, color: Colors.color000000, fontFamily: 'Raleway-Regular', includeFontPadding: false, fontSize: 14 }} placeholder={I18n.t('managerContentsInsertNameHint')} placeholderTextColor={Colors.colorC4C4C4}></TextInput>
                            ...
                            <View style={{ alignItems: 'center' }}>
                                {this.state.placeInfoDatas.map((item, index1) => (
                                    <View style={{ flexDirection: 'row', height: 48, width: '100%', marginTop: 12, }}>
                                        ...
                                        <View style={{ flex: index1 == 0 ? 1 : 0.7, borderRadius: 4, borderWidth: 1, borderColor: Colors.colorD0D0D0, justifyContent: 'center', marginLeft: index1 == 0 ? 0 : 15, paddingLeft: 18 }}>
                                            <TextInput value={item.placeInfo} onChangeText={(text) => this._PlaceInfoInsert({ type: 3, placeInfo: text, no: index1 })} style={{ padding: 0, margin: 0, color: Colors.color000000, fontFamily: 'Raleway-Regular', includeFontPadding: false, fontSize: 14 }} placeholder={""} placeholderTextColor={Colors.colorC4C4C4}></TextInput>
                        ...
                        <View style={{ marginTop: 20, paddingLeft: 16, paddingRight: 16 }}>
                            ...
                            <View style={{ borderRadius: 4, borderWidth: 1, borderColor: Colors.colorD0D0D0, justifyContent: 'center', paddingLeft: 18, height: 48, marginTop: 12 }}>
                                <TextInput style={{ padding: 0, margin: 0, color: Colors.color000000, fontFamily: 'Raleway-Regular', includeFontPadding: false, fontSize: 14 }} placeholder={I18n.t('managerContentsInsertAddressDetailHint')} placeholderTextColor={Colors.colorC4C4C4} onChangeText={(text) => this.setState({ addressDetail: text })} value={this.state.addressDetail}></TextInput>
                            </View>
                            ...
                            <View style={{ borderRadius: 4, borderWidth: 1, borderColor: Colors.colorD0D0D0, justifyContent: 'center', paddingLeft: 18, height: 48, marginTop: 12 }}>
                                <TextInput style={{ padding: 0, margin: 0, color: Colors.color000000, fontFamily: 'Raleway-Regular', includeFontPadding: false, fontSize: 14 }} placeholder={"https://"} placeholderTextColor={Colors.colorC4C4C4} onChangeText={(text) => this.setState({ placeWeb: text })} value={this.state.placeWeb}></TextInput>
                            </View>
                                        ...
                                        {index1 != 0 && (
                                            <View style={{ flex: 0.3, borderRadius: 4, borderWidth: 1, borderColor: Colors.colorD0D0D0, justifyContent: 'center' }}>
                                                <SelectDropdown
                                                    ...
                                                    defaultButtonText={(item.language == undefined || item.language.length == 0) ? I18n.t('managerContentsInsertLanguage') : this.state.lang == 'ko' ? (item.language == 'ko' ? '한국어' : item.language == 'en' ? '영어' : '일본어') : (item.language == 'ko' ? 'Korean' : item.language == 'en' ? 'English' : 'Japanese')}
                                                    onSelect={(selectedItem, index) => {
                                                        this._GoodsInsert({ type: 2, language: index == 0 ? 'ko' : index == 1 ? 'en' : 'ja', no: index1 })
                                                    }} />
                                            </View>)}

                                        <View style={{ flex: index1 == 0 ? 1 : 0.7, height: 48, borderRadius: 4, borderWidth: 1, borderColor: Colors.colorD0D0D0, paddingLeft: 12, justifyContent: 'center', marginLeft: index1 == 0 ? 0 : 15 }}>
                                            <TextInput value={item.name} onChangeText={(text) => this._GoodsInsert({ type: 3, name: text, no: index1 })} style={{ padding: 0, margin: 0, color: Colors.color000000, fontFamily: 'Raleway-Regular', includeFontPadding: false, fontSize: 14, }} placeholder={I18n.t('managerContentsInsertPriceHint')} placeholderTextColor={Colors.colorC4C4C4}></TextInput>
                                            {index1 > 0 && <TouchableOpacity style={{ position: 'absolute', right: 9 }} onPress={() => this._GoodsInsert({ type: 6, no: index1 })}>
                                                <View style={{ width: 25, height: 25, borderRadius: 20, backgroundColor: Colors.colorE94D3E, alignItems: 'center', justifyContent: 'center' }}>
                                                    <Image source={imgDelete} style={{ width: 12, height: 13, resizeMode: 'contain' }}></Image>
                                                </View>
                                            </TouchableOpacity>}
                                    ...
                                    <View style={{ flexDirection: 'row', height: 48, width: '100%', marginTop: 12, }}>
                                        <View style={{ flex: 0.3, borderRadius: 4, borderWidth: 1, borderColor: Colors.colorD0D0D0, justifyContent: 'center' }}>
                                            <SelectDropdown
                                                ...
                                                defaultButtonText={(item.currency == undefined || item.currency.length == 0) ? (this.state.lang == 'ko' ? '원화 (₩)' : 'KRW (₩)') : this.state.lang == 'ko' ? (item.currency == 'KRW' ? '원화 (₩)' : item.currency == 'USD' ? '달러화 ($)' : '엔화 (¥)') : (item.currency == 'KRW' ? 'KRW (₩)' : item.currency == 'USD' ? 'USD ($)' : 'JPY (¥)')}
                                                onSelect={(selectedItem, index) => {
                                        ...
                                        <View style={{ flex: 0.7, borderWidth: 1, borderColor: Colors.colorFFFFFF, borderBottomColor: Colors.colorD0D0D0, justifyContent: 'center', marginLeft: 12, paddingLeft: 4 }}>
                                            <TextInput value={item.price} keyboardType={Platform.OS == 'ios' ? 'number-pad' : 'numeric'} onChangeText={(text) => this._GoodsInsert({ type: 5, price: text.replace('-', '').replace('.', ''), no: index1 })} value={this.state.goodsDatas[index1].price} style={{ padding: 0, margin: 0, color: Colors.color000000, fontFamily: 'Raleway-Regular', includeFontPadding: false, fontSize: 14 }} placeholder={""} placeholderTextColor={Colors.colorC4C4C4}></TextInput>
                                        </View>
                                    </View>
                                </View>
                            ))} */}
                                ...
                                <View style={{ marginTop: index1 == 0 ? 12 : 24, borderWidth: 1, borderColor: Colors.colorD0D0D0, borderRadius: 4, padding: 8 }}>
                                    ...
                                    <View style={{ flexDirection: 'row', height: 48, width: '100%', marginTop: 12 }}>
                                        <View style={{ flex: 1, height: 48, borderRadius: 4, borderWidth: 1, borderColor: Colors.colorD0D0D0, paddingLeft: 12, justifyContent: 'center', marginLeft: 0 }}>
                                            <TextInput onChangeText={(text) => this._GoodsInsert({ type: 7, value: text, no: index1 })} style={{ padding: 0, margin: 0, color: Colors.color000000, fontFamily: 'Raleway-Regular', includeFontPadding: false, fontSize: 14, }} placeholder={I18n.t('managerContentsInsertPriceHint')} placeholderTextColor={Colors.colorC4C4C4}>{item.default}</TextInput>
                                        </View>
                                    </View>
                            ...
                            <View style={{ borderRadius: 4, borderWidth: 1, borderColor: Colors.colorD0D0D0, justifyContent: 'center', paddingLeft: 18, height: 48, marginTop: 12 }}>
                                <TextInput style={{ padding: 0, margin: 0, color: Colors.color000000, fontFamily: 'Raleway-Regular', includeFontPadding: false, fontSize: 14 }} placeholder={I18n.t('managerContentsInsertCreatorNameHint')} placeholderTextColor={Colors.colorC4C4C4} onChangeText={(text) => this.setState({ creatorName: text })} value={this.state.creatorName}></TextInput>
                            </View>
                            ...
                            <View style={{ borderRadius: 4, borderWidth: 1, borderColor: Colors.colorD0D0D0, justifyContent: 'center', paddingLeft: 18, height: 48, marginTop: 12 }}>
                                <TextInput style={{ padding: 0, margin: 0, color: Colors.color000000, fontFamily: 'Raleway-Regular', includeFontPadding: false, fontSize: 14 }} placeholder={"https://"} placeholderTextColor={Colors.colorC4C4C4} onChangeText={(text) => this.setState({ creatorUrl: text })} value={this.state.creatorUrl}></TextInput>
                            </View>
                                    ...
                                    {this.state.photoDatas.map((item, index) => (
                                        <TouchableWithoutFeedback>
                                            <View style={{ width: this.state.photoWidth / 5, height: this.state.photoWidth / 5, backgroundColor: Colors.colorFFFFFF, borderColor: Colors.colorB7B7B7, borderWidth: 1, borderRadius: 4, alignItems: 'center', justifyContent: 'center', marginLeft: 12, marginTop: index < 4 ? 0 : 12 }}>
                                                ...
                                                {/* <FastImage style={{ width: '100%', height: '100%', resizeMode: 'cover', borderRadius: 4 }} source={this.state.photoDatas.length > 0 ? { uri: item.uri, headers: { Authorization: 'someAuthToken' }, priority: FastImage.priority.normal } : null} resizeMode={FastImage.resizeMode.cover}></FastImage> */}
                            ...
                            <View style={{ marginTop: 12, flexDirection: 'row' }}>
                                ...
                                <TouchableOpacity>
                                    <View style={{ width: this.state.photoWidth / 5, height: this.state.photoWidth / 5, backgroundColor: Colors.colorFFFFFF, borderColor: Colors.colorB7B7B7, borderWidth: 1, borderRadius: 4, alignItems: 'center', justifyContent: 'center', marginLeft: 12 }}>
                                        ...
                                        {/* <FastImage style={{ width: '100%', height: '100%', resizeMode: 'cover', borderRadius: 4 }} source={this.state.coverDatas.length > 0 ? { uri: this.state.coverDatas[0].uri, headers: { Authorization: 'someAuthToken' }, priority: FastImage.priority.normal } : null} resizeMode={FastImage.resizeMode.cover}></FastImage> */}
                    ...
                    <FetchingIndicator isFetching={this.state.isFetching} message='' color={Colors.color2D7DC8} cancelable={true} cancelable={true} />
                </View>
            </SafeAreaView >
        )
