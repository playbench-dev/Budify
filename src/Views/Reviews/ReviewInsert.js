...
export default class ReviewInsert extends React.Component {
    constructor(props) {
        super(props)
    }
    ...
    _InsertReview() {
        ...
        url = ServerUrl.InsertReview

        formdata.append('order_no', this.props.route.params.data.orderNo)
        formdata.append('rate', this.state.score)
        formdata.append('content', this.state.contents)
        formdata.append('user_no', User.userNo)
        formdata.append('ex_no', this.props.route.params.data.exNo)
        ...
        return (
            <SafeAreaView>
                <View style={{ width: '100%', height: '100%', backgroundColor: Colors.colorFFFFFF, }}>
                    ...
                    <ScrollView>
                        <ImageBackground source={{ uri: ServerUrl.Server + this.props.route.params.data.pictureUrl }} style={{ width: '100%', height: screenWidth * 0.3692, marginTop: 12, alignItems: 'center', justifyContent: 'center' }}>
                            <Text style={{ fontFamily: 'Raleway-Bold', includeFontPadding: false, fontSize: 20, color: Colors.colorFFFFFF }}>{this.props.route.params.data.title}</Text>
                            <Text style={{ fontFamily: 'Raleway-Medium', includeFontPadding: false, fontSize: 14, color: Colors.colorFFFFFF, marginTop: 16 }}>{this.props.route.params.data.description}</Text>
                        </ImageBackground>
                        ...
                        <View style={{ width: '100%', paddingLeft: 16, paddingRight: 16, marginTop: 24 }}>
                            ...
                            <TouchableWithoutFeedback onPress={() => this._InsertReview()} disabled={this.state.score == 0 || this.state.contents.length == 0 ? true : false}>
